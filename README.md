...
* `clownmdemu` - requires to start with `SDL_RENDER_DRIVER=opengles2 clownmdemu` due to endianness issues with `SDL3` (should be fixed in `SDL3 3.4.0`)
...
* `jazz2-native` - Jazz Jackrabbit 2 requires OpenGL 3.3, needs to run via software rendering then `MESA_LOADER_DRIVER_OVERRIDE=llvmpipe jazz2`
